...
| 变量 | 默认 | 说明 |
|------|------|------|
| `CODEAGENT_VIDEO_GEN_PRESET_ID` | （空） | `video_generate` 工具使用的 preset id |
| `CODEAGENT_VIDEO_GEN_TIMEOUT_SEC` | `600` | 创建任务 + 轮询总超时（秒） |
| `CODEAGENT_VIDEO_GEN_POLL_INTERVAL_SEC` | `5` | 轮询任务状态间隔（秒） |
| `CODEAGENT_PUBLIC_BASE_URL` | （空） | 图生视频时 attachment 对外 URL 前缀（Agnes 需能访问） |

在 `config/seed.models.json` 中为 Agnes 视频模型设置 `"supports_video_gen": true`，`provider: agnes`，`model: agnes-video-v2.0`，`base_url: https://apihub.agnes-ai.com/v1`。Web UI compose 区可选择视频生成 preset；Agent 调用 `video_generate` 后 MP4 落盘为 attachment，tool trace 内嵌播放器预览。完整 API 说明见 [`AGNES_API.md`](AGNES_API.md)；官方文档：[Agnes Video V2.0](https://agnes-ai.com/doc/agnes-video-v20)。
...
配置页 **多模型预设** 按能力分组（对话 / 识图 / 生图 / 音频 / 音乐 / 视频生成 / 朗读）。每条 preset 对应一种用途，保存时 UI 自动写入 `use_type` 与 `supports_*`，一般无需手改 `config/seed.models.json`。
...
| 视频生成 | `video_gen` | `supports_video_gen: true` |
...
- [`AGNES_API.md`](AGNES_API.md) — Agnes AI Gateway API 概览与 CodeAgent 集成
