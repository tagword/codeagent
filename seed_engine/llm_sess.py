...
from seed_engine.config_plane import project_root
from seed_engine.models import Session
from seed_engine.sess_store import SessionStore
from codeagent.core.paths import agent_id_default, agent_home
...
def _project_sessions_dir(project_id: str, agent_id: Optional[str] = None) -> Path:
    """项目关联会话的存储目录。"""
    from codeagent.core.paths import agent_project_data_subdir
    aid = (agent_id or "").strip() or agent_id_default()
...
from codeagent.core.paths import agent_id_default
from seed_engine.models import Session
from seed_engine.sess_store import SessionStore
...
def _scrub_history_for_model(msgs: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    """
    try:
        from seed.agent_runtime import strip_inline_tool_markup_from_assistant_text as _strip
