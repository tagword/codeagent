...
def memory_search_handler(
    ...
    from src.mem_bridge_pkg import parsed_experience_project_id
    ...
    from src.codeagent_paths_pkg import agent_id_default, agent_memory_dir
            ...
            if exp_dir.is_dir():
                for p in sorted(exp_dir.glob("*.md"), key=lambda x: x.stat().st_mtime, reverse=True):
                    ...
                    if os.environ.get("CODEAGENT_MEMORY_SEARCH_INCLUDE_EXPIRED", "").lower() not in (
                        ...
                        try:
                            from src.mem_bridge_pkg import experience_file_expired
