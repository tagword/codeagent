...
def build_system_prompt(
    *,
    ...
    try:
        from src.codeagent.core.paths import agent_id_default, agent_persona_dir, ensure_agent_scaffold
    ...
    if filenames is not None:
        ...
        try:
            from src.ui_settings import get_system_prompt_filenames

            fnames = get_system_prompt_filenames(base=root)
        except Exception:
            fnames = list(CONFIG_FILENAMES)
...
def ensure_default_config_files(base: Optional[Path] = None) -> None:
    ...
    - Agent persona markdown (`agent.md`, `identity.md`, ...) is created under
      `agents/<id>/persona/` by `src.codeagent.core.paths.ensure_agent_scaffold()`.
...
plus optional ``config/skills/<plugin>.md`` when a plugin is enabled (see ``ui_settings``).
"""
